...
export function PropertiesTable({ properties }: { properties: Property[] }) {
  if (properties.length === 0) {
    return <p className="text-sm text-zinc-500">No properties found for this organization.</p>;
  }
        ...
        <tbody>
          {properties.map((p) => (
            <tr key={p.id} className="border-b border-zinc-50">
              <td className="py-3 pr-4 font-medium text-zinc-900">{p.name}</td>
              <td className="py-3 pr-4 text-zinc-600">{p.location ?? '—'}</td>
              <td className="py-3 pr-4 text-zinc-600">{p.unit_count}</td>
